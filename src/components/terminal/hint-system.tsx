...
const notes = [
  "Not every record wishes to be seen.",
  "Ordinary directory listings omit certain files.",
          ...
          {!guided && (
            <>
              ...
              {noteCount < notes.length ? (
                <button type="button" className="terminal-secondary" onClick={() => setNoteCount((count) => count + 1)}>Request archivist’s note</button>
              ) : !confirming ? (
                <button type="button" className="terminal-secondary" onClick={() => setConfirming(true)}>I require assistance from management</button>
              ) : (
