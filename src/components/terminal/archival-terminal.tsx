...
const ordinaryFiles = "archive  correspondence  ledger";
const hiddenFiles = ".  ..  .1926  archive  correspondence  ledger";
const maintenanceMemo = [
  "BLACK VEIL SYSTEMS MEMORANDUM / OCT. 1924",
  "Archive account: vale",
  "Credential policy: house flower; lowercase; no spaces.",
  "Objection: credential unchanged since opening.",
  "Disposition: objection denied. Mr. Vale dislikes needless expense.",
].join("\n");
...
export function ArchivalTerminal() {
  ...
  function outputFor(commandLine: string) {
    ...
    if (awaitingPassword) {
      ...
      if (trimmed.toLowerCase() === eventConfig.intermediateCredential) {
        setAuthenticated(true);
        ...
        append("error", "AUTHENTICATION REFUSED. The roses do not recognize you.");
      }
      return;
    }
    ...
    switch (command) {
      ...
      case "ls":
        if (args.includes("-la") || args.includes("-al") || (args.includes("-a") && args.includes("-l"))) {
          ...
          append("output", hiddenFiles);
          setProgress((value) => Math.max(value, 1));
        } else {
          append("output", ordinaryFiles);
        }
        break;
      case "pwd":
        append("output", cwd);
      ...
      case "cd": {
        ...
        if (["~", "/black-veil", "/black-veil/archive", "archive", ".."].includes(target)) {
          ...
          append("error", "cd: ledger: Permission denied");
        } else {
          append("error", `cd: ${target}: No such room or directory`);
        }
        break;
      }
      case "cat":
        if ([".1926", "/black-veil/archive/.1926"].includes(argument)) {
          ...
          append("error", "cat: a record must be named");
        } else {
          append("error", `cat: ${argument}: record unavailable`);
        }
        break;
      case "file":
      ...
      case "grep":
        ...
        break;
      case "unlock":
        if (args[0]?.toLowerCase() !== "ledger") {
          append("error", "unlock: specify a restricted record");
        } else if (args[1]) {
          if (args[1].toLowerCase() === eventConfig.intermediateCredential) {
            setAuthenticated(true);
            setProgress((value) => Math.max(value, 4));
            append("system", "AUTHENTICATION ACCEPTED\nWELCOME BACK, MR. VALE.\nLAST LOGIN: OCTOBER 31, 1924 · 11:47 P.M.\n1 UNREAD MESSAGE");
    ...
    <div className="terminal-and-help">
      <div className="terminal-window" onClick={() => inputRef.current?.focus()}>
        <div className="terminal-titlebar">
          ...
          <strong>BLACK VEIL ARCHIVAL SYSTEM</strong>
          <span>TTY 47</span>
        </div>
        <div className="terminal-output" ref={outputRef} aria-live="polite" aria-label="Terminal output">
          ...
          {isComplete && (
            <div className="access-card">
              ...
              <Link href="/guest-ledger">Enter the guest ledger</Link>
